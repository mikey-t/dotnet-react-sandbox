...
I'm utilizing a "swig cli module" to encapsulate functionality for all projects based on this template. This is essentially just an encapsulation of the exported functions that are simply re-exported from my swigfile. For more info on swig cli modules, see [https://github.com/mikey-t/swig-cli-modules](https://github.com/mikey-t/swig-cli-modules).
...
This project has some convenient wrapper commands to `dotnet ef` to help you manage DB migrations. Below are some example commands. Each Entity Framework command will require specifying a DbContext (because there's more than one: MainDbContext, TestDbContext). I've setup the wrapper commands so you can just pass `main`, `test` or `all` (defaults to `main` if you don't specify one).
...
All the commands above omitted which DbContext to use, which means it default to using `main`. You can also do any of the commands above to a specific DB (`main` or `test` or `all`):

```
npx swig dbListMigrations all
npx swig dbRemoveMigration test
npx swig dbAddMigration all YourMigrationName
...
- Step 3: run `npx swig dbAddMigration all MyNewTable`, which generates C# migration files and empty sql script files to add your sql to
...
- Step 10: Once you're happy with the code, apply it to the main database with `npx swig dbMigrate main` (or `all` if you haven't already applied it to `test`)
- Step 11: Apply any other desired changes to the repository before committing your code, and then commit it
- Step 12: Other developers can then checkout the latest code and run `npx swig dbMigrate all` to also get the latest DB changes for their local DB instances
